...
export function Review() {
  ...
  const [reviews, setReviews] = useState<Run[] | null>(null);
  const [error, setError] = useState<string | null>(null);
  const [starting, setStarting] = useState(false);

  const load = useCallback(async () => {
    try {
      const data = await apiFetch<Run[]>("/v1/runs?mode=literature_review");
      setReviews(data);
    } catch {
  ...
  async function onStart() {
    ...
    if (trimmed.length < 10) {
      setError("Describe the review topic in at least 10 characters.");
      return;
    }
    setStarting(true);
    ...
    <div className="mx-auto flex max-w-3xl flex-col gap-6">
      ...
      <Card className="flex flex-col gap-4 p-5">
        <Textarea
          ...
          placeholder="Example: Machine learning approaches for early sepsis prediction in intensive care"
          className="min-h-20"
        />
        <div>
          ...
          <Textarea
            ...
            placeholder="Example: Include only human studies with a clinical evaluation. Exclude preprints without peer review and studies before deep learning."
        ...
        <div className="flex flex-col gap-3">
          ...
          {showFilters && (
            <div className="flex flex-col gap-4 rounded-2xl border border-border p-4">
              ...
              <div className="flex flex-wrap items-center gap-3">
                <div className="w-40">
                  <p className="text-sm font-medium">Papers to review</p>
        ...
        <div className="flex justify-end">
          <Button onClick={onStart} loading={starting} disabled={!topic.trim()}>
            <ArrowUp /> Start literature review
          </Button>
        </div>
      </Card>
